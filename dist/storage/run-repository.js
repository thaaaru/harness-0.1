...
import { ApprovalDecisionSchema, AppSnapshotSchema, ExecutionResultSchema, HarnessRunInputSchema, RunRecordSchema, RunStatusSchema, TestPlanSchema, } from "../domain.js";
export class RunRepository {
    ...
    createRun(run) {
        ...
        this.db
            .prepare(`INSERT INTO runs (id, target_url, goal, status, input_json, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)`)
    ...
    saveSnapshot(runId, snapshot, savedAt) {
        const parsed = AppSnapshotSchema.parse(snapshot);
        this.db
            .prepare(`INSERT INTO snapshots (run_id, snapshot_json, saved_at)
         VALUES (?, ?, ?)
    ...
    appendEvent(runId, type, payload, createdAt) {
        this.db
    ...
    assertRunWasUpdated(runId, changes) {
